...
class PropertyGraphCobwebProcTest {
  ...
  void testCobwebSmall(final GraphDatabaseService db, final Dataset dataset) {
    try (Transaction tx = db.beginTx()) {
      final Stream<Node> nodes = tx.getAllNodes().stream();
      System.out.println(nodes.count());
      final PropertyGraphCobweb tree = PropertyGraphCobwebProc.integrate(tx.getAllNodes().stream())
          .findFirst().orElseThrow(() -> new RuntimeException("Unreachable"));
      Assertions.assertNotNull(tree);

      checkTree(tree, dataset);
    }
  }
  ...
  @GraphSource(dataset = Dataset.Rome99)
  ...
  /**
   * Tests cobweb using a large dataset for runtime errors.
   *
   * @param db      the database to be used
   * @param dataset the dataset to be used for testing & retrieving the number of nodes and edges
   */
  @Disabled
  //@Preprocessing(preprocessing = "MATCH (n) REMOVE n.nodeId RETURN n")
  @GraphSource(dataset = Dataset.RoadNetNY)
  @Test
   ...
   */
  //@Preprocessing(preprocessing = "MATCH (n) REMOVE n.nodeId RETURN n")
  @GraphSource(dataset = Dataset.InternetTopology)
  @Disabled
  @Test
  ...
  void checkTree(PropertyGraphCobweb tree, Dataset dataset) {
    final ConceptNode nodeTree = tree.getNodeTree();

    TreeUtils.printCutoffTrees(nodeTree);

    final List<String> ids = new ArrayList<>();

    this.checkIds(nodeTree, ids);
    this.checkPartitionCounts(nodeTree);
    this.checkParent(nodeTree);
    this.checkLeafType(nodeTree);

    Assertions.assertEquals(this.leafCount(nodeTree), dataset.getNodes());
  }

  /**
   * Checks if there are nodes that get duplicated during cobweb execution.
   * @param node the node currently inspected
   * @param ids the list of so far visited ids
   */
  ...
  /**
   * Tests if the merging branch of the cobweb function works as expected.
   */
  @Test
 void testMerge() {
    final ConceptNode root = new ConceptNode().root();
