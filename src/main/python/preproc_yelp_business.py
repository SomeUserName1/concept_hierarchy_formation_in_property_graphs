...
def prune(node, key):
    ...
    elif str(value).startswith("u'"):
        node[str(key)] = value[2:-1]
    elif value.startswith("'"):
        node[str(key)] = value[1:-1]

    if node[str(key)] is None or len(node[str(key)]) == 0 or value in false_values:
        del node[str(key)]
...
def main():
    with open(business, "r") as fread, open(preproc_business, "w") as fwrite:
        ...
        for line in fread:
            ...
            cats = []
            for category in node['categories'].split(","):
