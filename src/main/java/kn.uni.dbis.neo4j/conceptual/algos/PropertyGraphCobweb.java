...
public class PropertyGraphCobweb {
  ...
  private final ConceptNode nodeTree = new ConceptNode().root();
  ...
  public void integrateLabelPropStrcutChar(final Set<Node> nodes) {
    ...
    for (Node node : nodeList) {
      ...
      Cobweb.cobweb(conceptNode, this.nodeTree);
      i++;
      if (i % tenPercent == 0) {
        ++progress;
    ...
    TreeUtils.labelTree(this.nodeTree, "", "c");
  }
  ...
  public void integrateLabelStructChar(final Set<Node> nodes) {
    ...
    int progress = 0;
    int i = 0;
    int tenPercent = nodeList.size() < 10 ? 1 : (nodeList.size() / 10);
    for (Node node : nodeList) {
  ...
  public void integrateSubsequent(final Set<Node> nodes) {
    ...
    for (Node node : nodeList) {
      ...
      Cobweb.cobweb(summarizedNode, this.nodeTree);
      i++;
      if (i % tenPercent == 0) {
        ++progress;
        System.out.println(progress + "0% of the node summaries processed");
   ...
   */
  public ConceptNode getNodeTree() {
